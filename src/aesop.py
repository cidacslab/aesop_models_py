from ff_models import find_best_formula, lags_combination_vars, harmonic
import pandas as pd
import polars as pl
import numpy as np
import warnings
...
def compute_negbi_spc(serie: pl.Series):
    data: list[pl.Series] = []
    serie_4 = serie.rolling_mean(
        window_size=4,
        min_samples=1
    ).alias(f'{serie.name}_4')
    data.extend([serie, serie_4])
    for lag in range(1, 5):
        data.append(
            serie.shift(lag).alias(f'{serie.name}_lag_{lag}')
        )
    ...
    df = df.with_columns(
        ...
        pl.lit(model.conf_int().loc['time_trend', 0]).alias(f'IC_low_negbi_{serie_4.name}'),
        pl.lit(model.conf_int().loc['time_trend', 1]).alias(f'IC_high_negbi_{serie_4.name}'),
    ...
    return result.select("alarm").to_series().to_list(), result.select("limit").to_series().to_list()
